...
#define RXBUFFERLENGTH 168
#define AES128LENGTHINBYTES 16


const uint8_t defaultKeyAES[16] = { 0x01, 0x23 , 0x45 , 0x67 , 0x89 , 0xab , 0xcd , 0xef , 0x12 , 0x34 , 0x56 , 0x78 , 0x9a , 0xbc , 0xde , 0xf0 };
const int defaultInitVector[4] = {0x00000000, 0x00000000, 0x00000000, 0x00000000};

volatile uint8_t rxBuffer[RXBUFFERLENGTH] = { };

int main(){
  init_uart(); //for communication PT CT
  init_gpio(); //for trigger
  uint8_t cmd;
  uint8_t cipher_output[16];
  ...
  //set default init vector for AES coprocessor: ECB mode
  for (i = 0; i < 4; i++) init_vector[i] = defaultInitVector[i];
  set_baud(38400);
 ...
 //  set_baud(38400);
    ...
    for (i = 0; i < RXBUFFERLENGTH; i++) rxBuffer[i] = 0; //Zero the rxBuffer
        ...
        AES128_ECB_encrypt(rxBuffer, keyAES, rxBuffer + AES128LENGTHINBYTES); 
  ...
  // //swaes encryption
  // printf("SW_AES ENCRYPTION\n");
  // uint8_t key[16] = { 0x54, 0xee, 0xff, 0x11, 0x21, 0x32, 0xeb, 0x43, 0xbb, 0xaa, 0xff, 0x23, 0x98, 0x54, 0x12, 0x12};
  // printf("128-bit key: ");
  // for (int i = 0; i < 16; i++){
  // printf("%x", key[i]);
  // } 

  // uint8_t input[16] = { 0xca, 0xfe, 0xba, 0xbe, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0x65, 0x23, 0x67, 0x23 };
  // printf("\n128-bit input: ");
  // for (int i = 0; i < 16; i++){
  //   printf("%x", input[i]);
  // }
  ...
  // uint8_t de_out[16];
  // AES128_ECB_decrypt(output,key,de_out);
  // printf("\n128-bit output: ");
  // for (int i = 0; i < 16; i++){
  //   printf("%x", de_out[i]);
  // } 
